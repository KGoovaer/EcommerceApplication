...
<% if (listv == null || listv.isEmpty()) { %>
	<div class="container">
		<div class="w3-panel w3-pale-yellow w3-border" style="padding:20px; margin-top:20px;">
			...
			<p>Try a different search term or browse by <a href="categorya.jsp">category</a>.</p>
		</div>
	</div>
<% } else { %>
	<div class="container">
	...
	<% for (viewlist v : listv) { %>
		...
		<center>
			<table>
				<tr><th>
					<a href='selecteditema.jsp?Pn=<%= escHtml(v.getPimage()) %>'><img src=' images/<%= escHtml(v.getPimage()) %>' height=150px width=150px></a>
				</th></tr><br>
				<tr style='background-color: #ebe9eb'><th style='text-align: center'>
					<a href='selecteditema.jsp?Pn=<%= escHtml(v.getPimage()) %>'><%= escHtml(v.getBname()) %> <%= escHtml(v.getPname()) %></a>
				</th></tr>
